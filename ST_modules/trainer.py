...
from ST_modules.loss import SNNLoss, MarginLoss
...
class Trainer:
    def __init__(self, batch_size, network, device, augment_ops=None):
        ...
        # method section
        self.features_dw = 0.01
        self.snnl_weight = 0.0
        self.ce_weight = 0.0
        self.bce_weight = 1.0
        self.ss_weight = 0.0
        self.nno = True
        ...
        # tau
        self.deep_nno = False  # deep nno
        self.tau_val = not True
        self.multiple_taus = False
        if self.multiple_taus:
            self.tau = Parameter(torch.ones(2, device=device)*0.5, requires_grad=True)
        else:
            self.tau = Parameter(torch.tensor([0.5], device=device), requires_grad=True)

        if self.deep_nno or self.nno:
            self.deep_nno_handler = DeepNNO(self.tau, device, factor=2.0, bm=self.deep_nno)
    ...
    def train(self, epoch, train_loader, subset_trainloader, optimizer, class_dict, iteration):
        ...
        for idx, (inputs, targets_prep) in enumerate(train_loader):
            ...
            targets = torch.zeros(inputs.shape[0], len(np.unique(class_dict))).to(self.device) #pode dar problema aqui
        ...
        with torch.no_grad():
            for idx, (inputs, targets_prep) in enumerate(test_loader):
                ...
                if idx == 0:
                    ...
                    new_outputs_copy = new_outputs.detach().cpu().numpy()
                    sel_objs.extend(indices + self.batch * idx)
                    score_objs.extend(new_outputs_copy[indices, last_class])
                    '''
                    indices = [x for x in indices if targets_prep[x] == last_class]
                    if indices != [None] and indices!= []:
